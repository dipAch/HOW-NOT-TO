<?php
	...
	$err_file_size = false;
	$err_file_type = false;
	$err_file_format = false;
	$file_format_found = false;
	$FILE_FORMAT = array("PDF", "pdf");
	// File Checker Limit
	...
	if(!$error) {
		if(!($err_file_size)){
			if(!(preg_match("/application\/pdf/", $_FILES['pdffile']['type']))){
				$error = true;
				$err_file_type = true;
			}
		}
		if(!($err_file_size) && !($err_file_type)){
			/*list($width, $height, $type, $attr) = getimagesize($_FILES['pdffile']['tmp_name']);
			if($width > 3000) {
				$error = true;
				$err_pic_width = true;
			}
			...
			if(!($err_pic_width) && !($err_pic_height)) {*/
				...
				foreach ($FILE_FORMAT as $file_format) {
	...
	if (!$error) {
		...
		$dest_loc = "newsletter\\".basename($_FILES['pdffile']['name']);
		$temp_loc = $_FILES['pdffile']['tmp_name'];
		move_uploaded_file($temp_loc, $dest_loc);
		header("Location: createZIP.php?filename=$pdffile"); // ZIP It
...
<br /><br />
<div class="main_head">
									<span id = "error"><img src = "cross.png" width = "25" height = "25" valign = "middle" />&nbsp;&nbsp;<font size = "2" color = "red"><b>UPLOAD UNSUCCESSFUL</b></font></span>&nbsp;
							  </div>
<br /><br />
...
<table width="500" border="0" cellpadding="0" cellspacing="0" style="">
	<tr>
		<td> 
			...
			if($err_file_size){
				echo '<br><br><img src = "caution1.png" width = "15" height = "15" valign = "middle" />&nbsp;&nbsp;<font size = "1" color = "#FF0000"><b>FILE SIZE ERROR</b></font><br>';
			}
			if($err_file_type){
				echo '<br><br><img src = "caution1.png" width = "15" height = "15" valign = "middle" />&nbsp;&nbsp;<font size = "1" color = "#FF0000"><b>FILE NOT A PDF</b></font><br>';
			}
			if($err_file_format) {
				echo '<br><br><img src = "caution1.png" width = "15" height = "15" valign = "middle" />&nbsp;&nbsp;<font size = "1" color = "#FF0000"><b>FILE FORMAT NOT SUPPORTED</b></font><br>';
			}
			echo '<br />
