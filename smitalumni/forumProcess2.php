<?php
	...
	if(!empty($_POST['stories']) && strlen($_POST['stories']) < 1000) {
	...
	$name = $row['name'];
	$post = nl2br(htmlentities($_POST['stories'], ENT_QUOTES, 'UTF-8'));
	...
	if(!$longWord && strlen($post) > 35) { // If not too long
		$result1 = mysqli_query($cxn, "INSERT INTO stories (post, postedOn, postedBy, reg_no) VALUES('$post', '$now', '$name', '$reg')");
		if($result1) {
