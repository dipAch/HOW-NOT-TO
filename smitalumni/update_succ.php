<?php
	...
	if(!empty($_POST['ed_success']) && strlen($_POST['ed_success']) < 1000) {
	$post = nl2br(htmlentities($_POST['ed_success'], ENT_QUOTES, 'UTF-8'));
	...
	if(!$longWord && strlen($post) > 35) { // If not too long
		$result1 = mysqli_query($cxn, "UPDATE stories SET post = '$post' WHERE reg_no = '$reg' and post_id = $post_id");
		if($result1) {
