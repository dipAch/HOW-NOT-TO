<?php
	...
	if(!empty($_POST['ed_jobpos']) && strlen($_POST['ed_jobpos']) < 1000) {
	$post = nl2br(htmlentities($_POST['ed_jobpos'], ENT_QUOTES, 'UTF-8'));
	...
	if(!$longWord && strlen($post) > 35) { // If not too long
		$result1 = mysqli_query($cxn, "UPDATE jobs SET post = '$post' WHERE reg_no = '$reg' and post_id = $post_id");
		if($result1) {
