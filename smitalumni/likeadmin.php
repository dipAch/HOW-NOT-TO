<?php
	...
	$query = "SELECT likes FROM ad_post WHERE post_id = '$post_id'";
	$result = mysqli_query($cxn, $query);
	if($result) {
		$row = mysqli_fetch_row($result);
		$curr = $row[0] + 1;
		$query = "UPDATE ad_post SET likes = $curr WHERE post_id = '$post_id'";
		$result = mysqli_query($cxn, $query);
		if($result) {
			$query1 = "INSERT INTO like_acc VALUES($post_id, '$reg', 'adminPost')";
