<?php
	...
	if(!empty($_POST['contribute']) && strlen($_POST['contribute']) < 1000) {
	...
	$name = $row['name'];
	$post = nl2br(htmlentities($_POST['contribute'], ENT_QUOTES, 'UTF-8'));
	...
	if(!$longWord && strlen($post) > 35) { // If not too long
		$result1 = mysqli_query($cxn, "INSERT INTO feed (post, postedOn, postedBy, reg_no) VALUES('$post', '$now', '$name', '$reg')");
		if($result1) {
