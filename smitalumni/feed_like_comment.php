<?php
	...
	$query = "SELECT likes FROM feed_comment WHERE cid = $post_id";
	$result = mysqli_query($cxn, $query);
	if($result) {
		$row = mysqli_fetch_row($result);
		$curr = $row[0] + 1;
		$query = "UPDATE feed_comment SET likes = $curr WHERE cid = $post_id";
		$result = mysqli_query($cxn, $query);
		if($result) {
			$query1 = "INSERT INTO comment_like_acc VALUES($post_id, '$reg', 'contribute')";
